...
For every interface-constraint patch, the builder checks that its chain ID belongs to exactly one current protein card and that every 1-based residue index is within that chain's sequence length. Invalid interface cards are reported and omitted from generated YAML; prediction preflight repeats the checks for manually edited YAML.
