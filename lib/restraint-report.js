...
function summarizeModelAudit(model, measuredRestraints, measuredUnionGroups) {
...
async function createRestraintReport(resultDirectory, restraints, unionGroups = []) {
  ...
  return {
    ...
    model_summaries: parsedStructures.map((structure) => (
      summarizeModelAudit(structure.model, measuredRestraints, measuredUnionGroups)
    )),
...
async function writeRestraintReport(resultDirectory, restraints, unionGroups = []) {
  const report = await createRestraintReport(resultDirectory, restraints, unionGroups);
