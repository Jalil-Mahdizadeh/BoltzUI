...
// Boltz 2.2.1 boltz.data.const.method_types_ids, excluding reserved FUTURE slots.
...
const optionSchema = [
  ...
  { key: "model", flag: "--model", label: "Model", type: "select", group: "Execution", default: "boltz2", choices: ["boltz1", "boltz2"] },
  {
    ...
    choiceLabels: Object.fromEntries(METHOD_CHOICES.map((choice) => [choice, choice.toUpperCase()]))
  },
  ...
  { key: "addh", flag: "--addh", label: "Add hydrogens", type: "bool", group: "Output", default: false, exclusiveWith: "addh_energy_min" },
  { key: "addh_energy_min", flag: "--addh-energy-min", label: "Add hydrogens + energy minimize", type: "bool", group: "Output", default: false, exclusiveWith: "addh" },
...
function normalizeOptions(inputOptions = {}) {
  ...
  if (normalized.addh && normalized.addh_energy_min) {
    throw new Error("Add hydrogens and Add hydrogens + energy minimize are mutually exclusive.");
  }
  return normalized;
}
...
function resolvePredictionOptions(inputOptions = {}, presetId = DEFAULT_PRESET, context = {}) {
  ...
  const overrideWasExplicit = preset.id === "custom"
    && Object.prototype.hasOwnProperty.call(inputOptions, "override");
  if (context.hasAtomContacts && !overrideWasExplicit) options.override = true;
