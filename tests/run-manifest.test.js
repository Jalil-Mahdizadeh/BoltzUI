...
test("run manifest records resolved provenance and excludes credentials", () => {
  ...
  const manifest = createRunManifest({
    ...
    atomContacts: [],
    tokenContacts: [{
      chain1: "A", token1: 1, chain2: "B", token2: 2, max_distance: 6, force: true
    }],
    atomContactUnions: [{
      group_index: 1,
      force: true,
  ...
  assert.equal(manifest.token_contact_constraints.length, 1);
  assert.equal(manifest.interface_contact_constraints.length, 1);
